...
test-ledger-mock:
	@go test -mod=readonly `go list github.com/ivansukach/modified-cosmos-sdk/crypto` -tags='cgo ledger test_ledger_mock'

test-ledger: test-ledger-mock
	@go test -mod=readonly -v `go list github.com/ivansukach/modified-cosmos-sdk/crypto` -tags='cgo ledger'
...
format: tools
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/lcd/statik/statik.go" | xargs goimports -w -local github.com/ivansukach/modified-cosmos-sdk
.PHONY: format
